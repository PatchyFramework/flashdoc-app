  ...
  <div class="w-full p-4 space-y-5">
    ...
    <div class="relative shadow-md border-gray-300 border-opacity-50 border rounded overflow-y-auto max-h-[400px] scroll-smooth">
      <table class="w-full text-sm text-left text-black">
          ...
          <tbody>
            @foreach($janji as $j)
              <tr class="bg-white border-b" data-id="{{ $j['id'] }}">
                  ...
                  <td class="flex flex-row px-6 py-4 justify-center">
                    <button data-id="{{ $j['id'] }}" data-pasien="{{ $j->user->name }}" data-dokter-id="{{ $j->dokter->id }}" data-pasien-data="{{ $j->user->name }}" class="showEditModal block h-[25px] p-2 py-0 bg-tealblue rounded text-white font-bold mr-2">
                      <i class="fa-solid fa-pen-to-square"></i>
                    </button>                    
        

                    <button data-id="{{ $j->id }}" class="showDeleteModal block h-[25px] p-2 py-0 bg-danger rounded text-white font-bold">
